...
template<typename T>
Vector<4, T> Base::Math::QuaternionConjugate(Vector<4, T>& Q)
{ 
	return Vector<4, T>({
		-Q[0],
		-Q[1],
...
template<typename T>
Vector<4, T> Base::Math::QuaternionMultiply(Vector<4, T>& Q1, Vector<4, T>& Q2)
{
...
template<typename T>
Vector<4, T> Base::Math::QuaternionFromAxisAngle(Vector<3, T>& Axis, T Angle)
{
	Vector<4, T> retVector;
...
Matrix<4, T> Base::Math::RotationMatrixFromQuaternion(Vector<4, T>& Q)
{
